...
public class PlayerInventory implements IInventory {
    ...
    public ItemStack addItemStack(ItemStack item) {
        ...
        for(Slot slot : slots) {
            if(slot.getStack().matches(item)) {
                int extra = slot.addItemAmount(item.getAmount());
                if(extra <= 0) {
                    System.out.println("Added "+item.getAmount()+"x "+item.getItem().getName() + " to slot #"+slot.getSlotIndex());
                    return null;
                }else{
                    amt=extra;
                }
        ...
        System.out.println("Extra " + item.getAmount() + "x " + item.getItem().getName());
        return item;
    }
